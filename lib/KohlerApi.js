...
class KohlerApi {
  ...
  async startShower({
    ...
    // valve_num tells the controller how many valves are active.
    // Sending valve_num=2 when valve2 has no outlets causes "Paused" state.
    const valveNum = (valve2Outlet && valve2Outlet !== '0') ? 2 : 1;
    const params = {
      valve_num: valveNum,
    ...
    console.log(`[API] startShower params: ${JSON.stringify(params)}`);
    return this._request('quick_shower.cgi', params, TIMEOUT_COMMAND);
  }
  ...
  async lightOn(zone, level = 100) {
    return this._request('light_on.cgi', {
      zone,
  ...
  async lightOff(zone) {
    return this._request('light_off.cgi', { zone }, TIMEOUT_COMMAND);
  }
